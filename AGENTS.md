...
- Read `skill/SKILL.md` and focused files in `skill/references/` when deeper guidance is useful.
- Do not install, pass, or consume `dist/skill.zip` in coding agents.
- Ignore `skill/agents/openai.yaml`; it is ChatGPT Skill metadata only.
...
4. Rebuild the optional ChatGPT Skill package when files under `skill/` change.
...
8. Commit source changes under `skill/` and `dist/skill.zip` together when the package is rebuilt.
